...
from .utils.givensdecompose import _givens_decompose_column_parallel
...
class ParallelCliffordLoader(QuantumCircuit):
    r"""A circuit that prepares a Clifford loader with a logarithmic depth.
   ...
   Clifford loader is a state of the form
    ...
    Here .. math::\mathcal{U}(x_i) is implemented with parallel Givens rotations.
    ...
    Reference: `arXiv:2202.00054`_

    .. _arXiv:2202.00054: https://arxiv.org/pdf/2202.00054
    """
        ...
        **circuit_kwargs,
        ...
        if isinstance(qubit_mapper, JordanWignerMapper):
                for i in range(m-1,-1,-1): # over the columns in reverse order
                    # decomposition of U with Givens rotations
                    decomposition_reversed  = _givens_decompose_column_parallel(register, data_matrix[:,[i]])
                    ...
                    self.barrier(register) # only for nice looking drawings
                    self.append(XGate(), [register[0]])# Xgate represents (c_1 + c_1^*)
                    self.barrier(register) # only for nice looking drawings
                    # Append U
