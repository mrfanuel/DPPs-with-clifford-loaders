...
from .utils.givensdecompose import _givens_decompose_column_pyramid
...
class PyramidCliffordLoader(QuantumCircuit):
    r"""A circuit that prepares a Clifford loader with a linear depth.
        ...
        **circuit_kwargs,
        ...
        if isinstance(qubit_mapper, JordanWignerMapper):
                ...
                for i in range(m-1,-1,-1): # over the columns in reverse order
                    # decomposition of U                    
                    decomposition_reversed  = _givens_decompose_column_pyramid(register, data_matrix[:,[i]])
                    ...
                    # top of the pyramid representing (c_1 + c_1^*)
                    self.append(XGate(), [register[0]])
                    # Append U
